...
//
//    vector<Store> sortedStores;
//    for (auto* storePtr : storePtrs)
//        sortedStores.push_back(*storePtr);
//
//    stores = sortedStores;
    ...
    vector<const Store*> storePtrs;
        ...
        // Sort the vector of pointers using the comp parameter
        sort(storePtrs.begin(), storePtrs.end(), comp);

        // Rearrange the Stores in the correct order
      
        transform(storePtrs.begin(), storePtrs.end(), auxStores.begin(),
                  [](const Store* ptr) { return *ptr; });
